# Spec: config-proto-extension

## Purpose

Defines requirements for the `config-proto-extension` capability.
...
`pkg/logger.ZapLogger` SHALL provide a `Zap() *zap.Logger` method that returns the underlying `*zap.Logger` instance.

#### Scenario: Zap getter returns configured logger

- **WHEN** `NewLogger` is called with a dev config and `Zap()` is called on the result
- **THEN** the returned `*zap.Logger` SHALL use the same core, level, and options as the `ZapLogger`

#### Scenario: franz-go kzap uses Servora logger

- **WHEN** `kzap.New(zapLogger.Zap())` is used to create a franz-go logger
- **THEN** Kafka client internal logs SHALL be output through the same zap instance, respecting the configured log level and output targets (console/file)

### Requirement: pkg components follow Optional initialization pattern

All new pkg components (`pkg/broker/kafka`, `pkg/audit`) SHALL provide `NewXxxOptional` factory functions that:
- Accept proto config types (`*conf.Data`, `*conf.App`)
- Return nil (not error) when the component is not configured
- Log a message at Info level when skipping initialization
- Follow the same pattern as `pkg/openfga.NewClientOptional`

#### Scenario: NewBrokerOptional with nil config

- **WHEN** `NewBrokerOptional` is called with `cfg.Kafka == nil`
- **THEN** it SHALL return nil and log "Kafka not configured, broker disabled"

#### Scenario: NewRecorderOptional with nil audit config

- **WHEN** `NewRecorderOptional` is called with `cfg.Audit == nil`
- **THEN** it SHALL return a Recorder with NoopEmitter
