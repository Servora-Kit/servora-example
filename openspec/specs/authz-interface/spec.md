# Spec: authz-interface

## Purpose

Defines the pluggable authorization interface for `pkg/authz`, enabling multiple authorization engines (OpenFGA, noop) to be used interchangeably via a common `Authorizer` contract and `Server()` middleware.
