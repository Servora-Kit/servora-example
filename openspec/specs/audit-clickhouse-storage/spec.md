# Spec: audit-clickhouse-storage

## Purpose

Defines requirements for the `audit-clickhouse-storage` capability — ClickHouse schema design, idempotent DDL management, and the BatchWriter implementation for persisting audit events.
...
- `event_id String`, `event_type LowCardinality(String)`, `event_version String`
- `occurred_at DateTime64(3, 'UTC')`
- `service LowCardinality(String)`, `operation String`
...
The table SHALL use `PARTITION BY toDate(occurred_at)` for daily partitioning and `TTL occurred_at + INTERVAL N DAY` where N is read from `conf.App.Audit.retention_days` (default 90). The sort order SHALL be `ORDER BY (service, event_type, occurred_at, event_id)`.
