# Spec: authn-interface

## Purpose

Defines the pluggable authentication interface for `pkg/authn`, enabling multiple authentication engines (JWT, header-based, noop) to be used interchangeably via a common `Authenticator` contract and `Server()` middleware.
